...
- 외부 GitHub Action은 전체 커밋 SHA로 고정하고 checkout 자격 증명을 보존하지
  않는다. GitHub Actions의 모델 기반 review는 contextual-orchestrator의
  `orchestrator/free`만 사용한다. 해당 review workflow는
  provider·concrete model·group·paid fallback을 선택하지 않고 gateway token만
  사용하며, 가용한 free capability가 없으면 fail closed 한다. 별도의 비모델 NIM
  proposal sentinel은 `NVIDIA_NIM_API_KEY` 구성 여부만 확인할 수 있으며 값을 읽거나
  모델·publisher·review verdict를 실행하지 않는다.
