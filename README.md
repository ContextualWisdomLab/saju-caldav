...
- 두 사람의 출생 정보를 한 화면에 입력해 관계 작용 시간 또는 공통 조건 시간 확인
- 기본 생활 시간(09:00–23:00) 추천과 새벽을 포함하는 24시간 검색 선택
- 관계 작용 모드에서는 두 사람의 일지와 후보 날짜·시간 지지가 양쪽을 함께 잇는지
  계산하고, 각자의 개인 점수와 관계 작용 점수를 분리 표시
- 후보마다 일지·일간의 합/오행, 날짜·시간 지지 관계와 오행 흐름을 설명하고,
  네 연결 중 긍정·충·생극 흐름의 개수를 별도 지표로 표시
- 공통 조건 모드에서는 한 사람에게만 유리한 후보를 줄이고, 두 개인 점수의 균형을
  설명 가능한 기준으로 표시
...
첫 화면의 두 사람 입력 영역에서 저장된 프로필을 선택하거나 생년월일과 탄생 시를
바로 입력하면 됩니다. `찾을 시간의 의미`는 기본적으로 `두 사람 사이의 관계 작용
시간`이며, 필요하면 `각자에게 무난한 공통 시간`으로 바꿀 수 있습니다. 관계 작용
모드는 두 사람의 일지 관계와 후보 날짜·시간의 양쪽 연결을 기준으로 정렬하고,
공통 조건 모드는 두 사람의 개인 조건 교집합을 기준으로 정렬합니다. 두 모드 모두
프로필 시간대의 지금 이후부터 오늘 날짜에 365일을 더한 종료일까지(양끝 포함)
날짜마다 가장 높은 시간 하나를 골라 가까운 순으로 보여 줍니다.
...
양력 또는 한국 음력 생일로 일주를 계산해 선택한 두 사람 시간 모드의 후보를 계속
제공하지만, 시주는 임의로 추정하지 않습니다. 이 프로필은 출생 시주를 참조하는
고급 조건만 사용할 수 없으며, 현재 시간의 천간·지지를 직접 고르는 조건은 그대로
사용할 수 있습니다.

두 사람 시간 모드는 전통적인 궁합·길일 판정이 아닙니다. `관계 작용 시간`은
두 사람의 일지를 먼저 비교하고 후보 지지가 양쪽을 함께 잇는지 계산합니다.
`공통 조건 시간`은 각 사람의 일지와 후보 지지를 따로 비교해 교집합을 찾습니다.
결과에는 관계 작용 점수와 개인 점수를 구분해 표시합니다. 성별에 따른 배우자성·
대운 해석은 포함하지 않고 성별은 기록용으로만 보존합니다.
...
[배포 가이드](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/DEPLOYMENT.md)에 있습니다.

Keyverse를 연결할 때에는 [OIDC 연동 경계](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/security/KEYVERSE.md)의 exact
...
- [역법 조사와 구현 규칙](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/research/README.md)
- [아키텍처와 위협 경계](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/ARCHITECTURE.md)
- [ADR-0001: 런타임 잠금과 CI 출처](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/adr/0001-runtime-lock-and-ci-provenance.md)
- [ADR 색인과 개인정보 삭제 결정](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/adr/README.md)
- [위협 모델](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/security/THREAT_MODEL.md) · [테스트 전략](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/testing/TEST_STRATEGY.md)
- [Keyverse OIDC 연동](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/security/KEYVERSE.md)
- [운영성과](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/operations/OPERABILITY.md) · [요구사항 추적](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/TRACEABILITY.md)
- [배포와 CalDAV 연결](https://github.com/ContextualWisdomLab/saju-caldav/blob/main/docs/DEPLOYMENT.md)
