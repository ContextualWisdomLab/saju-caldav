...
양력 또는 한국 음력 출생 시각에서 사주 네 기둥을 계산하고, 두 사람의 공통
관계 기준을 함께 참고할 현재 이후의 실제 날짜·시각이나 사용자가 만든 조건의 일치 시간을
...
이후부터 오늘 날짜에 365일을 더한 종료일까지(양끝 포함) 날짜마다 두 사람의
관계 기준이 가장 균형 있게 겹치는 시간 하나를 골라 가까운 순으로 보여 줍니다.
...
양력 또는 한국 음력 생일로 일주를 계산해 공통 관계 후보를 계속 제공하지만, 시주를
임의로 추정하지 않습니다. 이 프로필은 출생 시주를 참조하는 고급 조건만 사용할
수 없으며, 현재 시간의 천간·지지를 직접 고르는 조건은 그대로 사용할 수 있습니다.

두 사람 후보는 전통적인 궁합·길일 판정이 아닙니다. 현재 방식은 두 사람의 일지와
후보 날짜·시간 지지의 관계표를 대칭적으로 정렬하는 문화적 참고 규칙입니다. 성별에
따른 배우자성·대운 해석은 포함하지 않고 성별은 기록용으로만 보존합니다.
