...
선택적 NIM 제안 단계는 다음 조건이 모두 맞을 때만 bounded patch를 만든다.
...
`gate → sentinel → (NIM proposal) → uncredentialed verify → bounded PR →
current-head checks/review → human merge`
...
NIM secret, maintainer 앱 자격 증명, 중앙 workflow 재사용 계약이 이 저장소에
명시적으로 제공될 때까지 이 ADR은 Proposed로 남긴다. 그 전에는 sentinel만
활성화해도 품질 감시는 유지된다.
