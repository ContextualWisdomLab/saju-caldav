...
- 규칙은 임의 코드나 식을 평가하지 않습니다. 8개 이하의 허용 필드 비교만
  역직렬화합니다.
- 두 사람 시간 검색은 두 프로필의 출생 일지와 후보의 날짜·시간 지지를 고정된
  허용 관계표로 비교합니다. 기본 `pair_relation_activation` 모드는 두 사람의
  일지 관계와 후보 지지의 양쪽 연결을 계산하고 `method`·`interpretation`에 같은
  식별자를 반환합니다. `shared_branch_relations` 모드는 각 사람의 개인 조건을
  따로 비교해 공통 조건을 찾고 `method=balanced_branch_harmony`,
  `interpretation=shared_branch_relations`를 반환합니다. 어느 모드도 전통적인
  궁합이나 ‘두 사람의 합’의 해석을 주장하지 않으며 `gender_policy=record_only`로
  성별을 계산에 쓰지 않습니다. 두 모드 모두 육충 후보를 제외하고, 첫 번째
