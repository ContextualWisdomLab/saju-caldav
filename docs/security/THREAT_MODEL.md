...
현재 endpoint와 회귀 테스트는 원격 삭제 결과와 로컬 행 부재를 확인하지만, 별도
삭제 감사 이벤트와 백업 보존·삭제 자동화는 아직 구현하지 않았다. 향후 감사
이벤트를 추가할 때에는 profile ID를 원문 이름·생년월일 대신 불가역 식별자로
남기고, 감사 이벤트 자체의 보존·삭제 주기를 ADR로 결정한다.
